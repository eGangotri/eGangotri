...
Total Pages:${delimiter}${formatInteger(TOTAL_NUM_PAGES)}"""
    ...
    static String folderNameToCenterName(String folderName) {
        for (Map.Entry<String, String> e : CENTER_MAP.entrySet()) {
            if (folderName.containsIgnoreCase("${File.separator}${e.key}")) {
                return e.value
            }
        }
        return folderName
